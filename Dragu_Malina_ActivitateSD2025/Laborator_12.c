...
void inserareInLP(NodLP** cap, Masina info)
{
...
void inserareMuchie(NodLP* cap, int idStart, int idStop)
{
	...
	if (nodStart != NULL && nodStop != NULL)
	{
		...
		inserareInLS(&(nodStop->vecini), nodStart); // Doar pentru graf neorientat
	}
}
...
NodLP* citesteGrafDeMasini(const char* numeFisierMasini, const char* numeFisierMuchii)
{
	NodLP* graf = citireNoduriMasiniDinFisier(numeFisierMasini);
	citireMuchiiDinFisier(graf, numeFisierMasini);
}

void dezalocareListaSecundara(NodLS* cap)
{
	while (cap != NULL)
	{
		NodLS* temp = cap->next;
...
void dezalocareNoduriGraf(NodLP** cap) 
{
	...
	{
		NodLP* temp = (*cap)->next;
		dezalocareListaSecundara((*cap)->vecini);
		free((*cap)->info.model);
...
int main() 
{
	NodLP* graf = citesteGrafDeMasini("masini_sortate.txt", "muchii.txt");
