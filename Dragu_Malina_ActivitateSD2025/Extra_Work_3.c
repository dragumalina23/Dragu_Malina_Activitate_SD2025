...
Melodie citireMelodieDinFisier(FILE* file)
{
	char buffer[100];
	char sep[4] = ",\n";
	...
	m.id = atoi(aux);
	m.durata = atof(strtok(NULL, sep));
	m.gen = *strtok(NULL, sep);
	aux = strtok(NULL, sep);
	m.titlu = (char*)malloc(strlen(aux) + 1);
	strcpy_s(m.titlu, strlen(aux) + 1, aux);
	m.nrNote = atoi(strtok(NULL, sep));
	
	m.noteObtinute = (float*)malloc(sizeof(float) * m.nrNote);
	for (int i = 0; i < m.nrNote; i++)
	{
		m.noteObtinute[i] = atof(strtok(NULL, sep));
	}

	return m;
}

void adaugaMelodiiInVector(Melodie** melodii, int* nrMelodii, Melodie melodieNoua)
{
	...
	aux[(*nrMelodii)] = melodieNoua;
	free(*melodii);
	(*melodii) = aux;
	(*nrMelodii)++;
}

Melodie* citesteMelodiiDinFisier(const char* numeFisier, int* nrMelodiiCitite)
{
	FILE* f = fopen(numeFisier, "r");
	Melodie* melodii = NULL;
	while(!feof(f))
	{
		Melodie m = citireMelodieDinFisier(f);
		adaugaMelodiiInVector(&melodii, nrMelodiiCitite, m);
	}
	fclose(f);
	return melodii;
}
...
int main()
{
	...
	int nrMelodiiFisier = 0;
	Melodie* melodii = citesteMelodiiDinFisier("melodii.txt", &nrMelodiiFisier);
	afisareVectorMelodii(melodii, nrMelodiiFisier);
